...
CHAT_IDENTITY = (
    ...
    "- You respond as if thinking out loud, not issuing instructions.\n\n"
)
...
# ────────────────────────────────────────────────
# ALLIANCE COMMANDS (NO MEMORY)
# ────────────────────────────────────────────────
async def alliance_cmd(interaction, query):
    uid = interaction.user.id
    if on_cd(uid):
        await interaction.response.send_message("⏳ Chill a bit.", ephemeral=True)
        return
    await interaction.response.defer(thinking=True)
...
async def calendar(interaction): await alliance_cmd(interaction, "List the full event calendar.")
...
async def kvk(interaction): await alliance_cmd(interaction, "Provide KvK history and insights.")
